...
export const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <div className={`relative overflow-hidden pt-28 sm:pt-32 ${dark ? 'bg-slate-900 text-white' : 'bg-gradient-to-b from-amber-500/5 via-slate-50 to-white text-slate-900'} border-b border-slate-200/60`}>
      {/* Background ambient solar blur */}
      <div className="absolute top-0 right-1/4 w-64 sm:w-96 h-64 sm:h-96 bg-amber-400/15 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 left-1/4 w-52 sm:w-80 h-52 sm:h-80 bg-orange-400/15 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className={`relative inline-block w-full max-w-4xl ${isCenter ? 'mx-auto' : ''}`}>
          <div 
            className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 max-w-2xl h-36 sm:h-44 ${
              dark 
                ? 'bg-gradient-to-r from-amber-500/20 via-orange-500/25 to-amber-500/15' 
                : 'bg-gradient-to-r from-amber-400/20 via-orange-400/25 to-amber-300/20'
          ...
          <h1 className={`relative text-3xl sm:text-4xl md:text-5xl font-serif font-extrabold tracking-tight leading-tight max-w-3xl ${isCenter ? 'mx-auto' : ''} ${
            dark 
              ...
              : 'drop-shadow-[0_2px_18px_rgba(245,158,11,0.15)]'
          }`}>
            {title}{' '}
            {highlightText && (
              <span className="bg-gradient-to-r from-amber-500 via-amber-600 to-orange-500 bg-clip-text text-transparent drop-shadow-[0_4px_24px_rgba(245,158,11,0.35)]">
                {highlightText}
              </span>
            )}
          </h1>
