...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  return (
    <Link
      ...
      className="group block bg-white rounded-2xl border border-slate-200/90 overflow-hidden shadow-sm hover:shadow-xl hover:border-amber-400/80 transition-all duration-300 cursor-pointer"
    >
      {/* Top Image Container */}
      <div className="relative h-64 sm:h-72 overflow-hidden bg-slate-900">
        ...
        <div className="absolute top-3.5 inset-x-3.5 flex items-center justify-between pointer-events-none">
          ...
          <span className="bg-amber-500 text-slate-950 font-extrabold text-xs px-3 py-1 rounded-full shadow-md">
            {project.systemSize}
          </span>
        </div>
      ...
      <div className="p-5 sm:p-6 flex flex-col justify-between">
        <h3 className="text-lg sm:text-xl font-serif font-bold text-slate-950 group-hover:text-amber-600 transition-colors duration-200 line-clamp-2 leading-snug">
          {project.title}
        </h3>

        {/* Bottom CTA Row */}
        <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 group-hover:text-amber-600 transition-colors">
